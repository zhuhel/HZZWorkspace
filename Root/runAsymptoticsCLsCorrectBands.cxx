...
bool conditionalExpected   = 0 && !doBlind; // Profiling mode for Asimov data: 0 = conditional MLEs, 1 = nominal MLEs
...
int maxRetries             = 5;             // number of minimize(fcn) retries before giving up
        ...
        stringstream* out_ss)
{
  ...
  TStopwatch timer;
  timer.Start();
  if (!ws_ || !mc_ || !data_ || !firstPOI_) {
    std::cout<<"missing stuff:"<<
      " ws="<<ws_<<
      " mc="<<mc_<<
      " data="<<data_<<
      " poi="<<firstPOI_<<std::endl;
    return;
  }
  w = ws_;
  mc = mc_;
  ...
  // !!!! Config the Minimization outside the code!!!
  ...
  std::cout<<"have asimov data:"<<std::endl;
  asimovData_0->Print();

  // w->writeToFile("ws_temp.root");
  ...
  if (betterBands && doExp) // no better time than now to do this
  {
      ...
      for (int N=2;N>=-2;N--)
      {
          ...
          RooDataSet* asimovData_N = makeAsimovData(conditionalExpected, asimov_0_nll, NtimesSigma, firstPOI_->GetName(), &muStr, &muStrPr, pr_val, 0);
  ...
  w->loadSnapshot("conditionalNuis_0");
  // setup the range of POI
  firstPOI->setRange(0, 1000);
...
double getLimit(RooNLLVar* nll, double initial_guess)
{
    ...
    if (muhat < 1E-6 || initial_guess != 0) setMu(initial_guess);
    ...
    while (fabs(mu_pre-mu_guess) > precision*mu_guess*direction)
    {
        ...
        cout << "guessed sigma" << sigma_guess << endl;
        ...
        const char* muName, string* mu_str, string* mu_prof_str, double mu_val_profile, bool doFit)
{
    ...
    cout << "Creating asimov data at "<< muName<<" = " << mu_val << ", profiling at "<<muName<<" = " << mu_val_profile << endl;
    ...
    if (doConditional && doFit)
    {
        ...
        cout << "Do not profile nuisances." << endl;
    }
    mu->setConstant(0);
    mu->setVal(mu_val);
