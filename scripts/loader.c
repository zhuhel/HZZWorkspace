        ...
        const char* x_title, bool shape_only, bool is_log, bool color_me) 
{
    ...
    GetMaxMin(histograms, max_temp, min_temp);
    double max_y = shape_only?h1->GetBinContent(h1->GetMaximumBin()):max_temp;
    double min_y = shape_only?GetMinOfHist(h1):min_temp;
    if (is_log) {
        canvas->SetLogy();
        if(fabs(min_y) < 1E-6) min_y = 1e-4;
        max_y *= 10;
        min_y /= 10;
    } else {
        max_y *= 1.1;
    ...
    while ((hist = (TH1*) next())) {
        ...
        if(icount == 0){
            ...
            hist->Draw();
            intgral_first_hist = hist->Integral(); 
        } else {
            if (shape_only) {
                hist->Scale(intgral_first_hist/hist->Integral());
            }
            hist->Draw("same");
        }
        legend->AddEntry(hist, tags.at(icount).c_str(), "F");
        icount += 1;
    }
        ...
        const vector<string>& tag_name)
{
    ...
    for(int ihist = 0; ihist < hist_names->size(); ihist++){ 
        ...
        compare_hists(*list, tag_name, hist_name, shape_only, is_log, color_me);
        clean_TList(*list);
        delete list;
    }
    delete hist_names;
